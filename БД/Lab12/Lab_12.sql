...
insert into AUDITORIUM values('301-1','ЛБ-К','5','204-1');
delete from AUDITORIUM where AUDITORIUM = '991-1';

begin try
	begin tran
		delete AUDITORIUM where AUDITORIUM = '301-1';
		insert into AUDITORIUM values('991-1','ЛБ-К','5','204-1');
		update AUDITORIUM set AUDITORIUM_CAPACITY = '30' where AUDITORIUM='301-1';
	commit tran;
end try
...
-- A ---
	...
	select @@SPID 'SID', 'update AUDITORIUM'  'результат',  AUDITORIUM_NAME, 
                      AUDITORIUM_TYPE,AUDITORIUM_CAPACITY from AUDITORIUM   where  AUDITORIUM='301-1';
	commit; 
	-------------------------- t2 -----------------
...
-- A ---
	...
	-------------------------- t1 --------------------
    select @@SPID 'SID' update AUDITORIUM set AUDITORIUM_CAPACITY = '30' where AUDITORIUM='301-1';	
	select @@SPID 'SID', 'update AUDITORIUM'  'результат',  AUDITORIUM_NAME, 
                      AUDITORIUM_TYPE,AUDITORIUM_CAPACITY from AUDITORIUM   where  AUDITORIUM='301-1';
...
begin tran
		INSERT into SUBJECT values('СТПИ','Современные технологии программирования в  интернете','ИСиТ');
		begin tran
		update SUBJECT set SUBJECT_NAME='СТПИ 1' where SUBJECT='СТПdaИ';
		commit
